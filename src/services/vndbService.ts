...
export const queryVNs = async (options: QueryOptions): Promise<VNDBResponse<VN>> => {
  const { page = 1, sort = 'rating', reverse = true, search, filters, results = 20 } = options;
  ...
  const response = await fetch(`${BASE_URL}/vn`, {
    ...
    body: JSON.stringify({
      ...
      fields: 'id, title, alttitle, image.url, image.sexual, image.violence, description, rating, votecount, length_minutes, released, platforms, languages, developers.id, developers.name',
...
export const getVN = async (id: string): Promise<VN | null> => {
  const response = await fetch(`${BASE_URL}/vn`, {
    ...
    body: JSON.stringify({
      filters: ['id', '=', id],
      fields: [
        'id', 'title', 'alttitle', 'aliases', 'olang', 'languages', 'platforms',
        'image.id', 'image.url', 'image.sexual', 'image.violence',
        'description', 'rating', 'votecount', 'length_minutes', 'released',
        ...
        'tags.id', 'tags.name', 'tags.rating', 'tags.spoiler', 'tags.lie', 'tags.category',
      ].join(', '),
      results: 1,
    }),
  });
...
export const getVNPublishers = async (vnId: string): Promise<{ id: string; name: string; original: string | null }[]> => {
  try {
    const response = await fetch(`${BASE_URL}/release`, {
      ...
      body: JSON.stringify({
        ...
        fields: 'producers.id, producers.name, producers.original, producers.publisher',
        results: 10,
      }),
    });
...
export const getVNCharacters = async (vnId: string): Promise<VNCharacter[]> => {
  try {
    const response = await fetch(`${BASE_URL}/character`, {
      ...
      body: JSON.stringify({
        filters: ['vn', '=', ['id', '=', vnId]],
        fields: 'id, name, original, aliases, description, image.url, image.sexual, image.violence, age, sex, height, weight, blood_type, traits.id, traits.name, traits.spoiler, vns.id, vns.role, vns.spoiler',
        results: 25,
        sort: 'id',
      }),
    });
    ...
    return (data.results || []).map((c: any) => ({
      ...
      role: c.vns?.find((v: any) => v.id === vnId)?.role ?? c.vns?.[0]?.role ?? 'appears',
    }));
  } catch (e) {
...
export const fetchUlist = async (
  ...
  const response = await fetch(`${BASE_URL}/ulist`, {
    ...
    body: JSON.stringify({
      user: userId,
      fields: 'id, added, vote, vn.id, vn.title, vn.alttitle, vn.image.url, vn.image.sexual, vn.image.violence, vn.rating, vn.votecount, vn.released, vn.length_minutes, labels.id, labels.label',
      results: 50,
      page,
    }),
  });
...
export const removeFromUlist = async (token: string, vnId: string): Promise<void> => {
  const response = await fetch(`${BASE_URL}/ulist/${vnId}`, {
    method: 'DELETE',
    headers: { Authorization: `Token ${token}` },
  });
...
export const getVNReleases = async (vnId: string): Promise<import('../types').Release[]> => {
  try {
    const response = await fetch(`${BASE_URL}/release`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...
        fields: [
          'id', 'title', 'alttitle',
          'languages.lang', 'languages.title', 'languages.latin', 'languages.mtl', 'languages.main',
          'platforms',
          'released',
          'minage',
          ...
          'voiced',
          'notes',
          'producers.id', 'producers.name', 'producers.original', 'producers.developer', 'producers.publisher',
...
export const queryReleases = async (opts: ReleaseQueryOptions): Promise<import('../types').VNDBResponse<import('../types').Release>> => {
  const { search, page = 1, results = 25, sort = 'released', reverse = true, filters = [] } = opts;

  const allFilters: any[] = [...filters];
  if (search) allFilters.push(['search', '=', search]);

  const finalFilter =
    allFilters.length === 0 ? ['id', '>=', 'r1']
    : allFilters.length === 1 ? allFilters[0]
    : ['and', ...allFilters];
  ...
  const response = await fetch(`${BASE_URL}/release`, {
    ...
    body: JSON.stringify({
      ...
      fields: [
        'id', 'title', 'alttitle',
        'languages.lang', 'languages.title', 'languages.latin', 'languages.mtl', 'languages.main',
        'platforms', 'released', 'minage',
        'patch', 'freeware', 'uncensored', 'official', 'has_ero',
        'engine', 'voiced', 'resolution',
        'producers.id', 'producers.name', 'producers.developer', 'producers.publisher',
        'extlinks.url', 'extlinks.label', 'extlinks.name',
        'vns.id', 'vns.title', 'vns.alttitle', 'vns.image.url', 'vns.image.sexual', 'vns.image.violence', 'vns.rtype',
      ].join(', '),
