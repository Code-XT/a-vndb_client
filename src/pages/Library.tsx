...
const NoImagePlaceholder: React.FC<{ title: string }> = ({ title }) => {
  const idx =
    title.split("").reduce((a, c) => a + c.charCodeAt(0), 0) % PALETTES.length;
...
const LibraryCard: React.FC<{ entry: UlistEntry }> = ({ entry }) => {
  ...
  const vn = entry.vn;
  const statusLabel = entry.labels?.find((l) => l.id >= 1 && l.id <= 4);
  const hasWishlist = entry.labels?.some((l) => l.id === 5) ?? false;
  const hasBlacklist = entry.labels?.some((l) => l.id === 6) ?? false;
  ...
  const statusColor =
    STATUS_META.find((s) => s.id === statusLabel?.id)?.color ?? "#6366f1";
    ...
    <Link to={`/vn/${entry.id}`} className="block">
      ...
      >
        ...
        <div className="relative aspect-[3/4] bg-slate-200 overflow-hidden flex-shrink-0">
          ...
          {/* VN rating — top right */}
          {vn.rating && (
            <div className="absolute top-1.5 right-1.5 flex flex-col items-end gap-1 text-[10px] font-bold text-white drop-shadow pointer-events-none">
          ...
          {/* List icons - top left */}
          {(hasWishlist || hasBlacklist) && (
            <div className="absolute top-1.5 left-1.5 flex flex-col items-start gap-1 drop-shadow pointer-events-none">
              {hasWishlist && (
                <div className="bg-purple-500/80 backdrop-blur-sm p-1 rounded-full text-white">
                  <Heart size={10} className="fill-white" />
                </div>
              )}
              {hasBlacklist && (
                <div className="bg-slate-700/80 backdrop-blur-sm p-1 rounded-full text-white">
                  <Ban size={10} />
                </div>
              )}
            </div>
...
export const Library: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchUlist(token, user.id, page)
      ...
      .catch(() =>
        setError("Failed to load library. Check your token in Profile."),
      )
  ...
  const cnt = (id: number) =>
    entries.filter((e) => e.labels?.some((l) => l.id === id)).length;
  const filtered = entries.filter(
    (e) => filter === "all" || e.labels?.some((l) => l.id === filter),
  );
      ...
      {entries.length > 0 && (
        <div className="flex gap-2 mb-5 overflow-x-auto scrollbar-hide pb-1">
          ...
          >
            ...
            All
            <span
              className={`tabular-nums text-[10px] ${filter === "all" ? "text-slate-400" : "text-slate-300"}`}
            >
              {entries.length}
            </span>
          </button>
          ...
          {STATUS_META.map((s) => {
            ...
            return (
              ...
              >
                ...
                {s.label}
                <span
                  className={`tabular-nums text-[10px] ${active ? "text-white/70" : "text-slate-300"}`}
                >
                  {c}
                </span>
              </button>
